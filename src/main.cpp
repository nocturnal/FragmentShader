...
InitLibPath()
{
    wxFileName lib = wxStandardPaths::Get().GetExecutablePath();
    ...
    if(lib.DirExists())
    {
        g_LibPath = lib.GetFullPath();
    }
    else
    {
        wxDirDialog picker (NULL, wxT("Locate lib folder"), lib.GetFullPath(), wxDD_DEFAULT_STYLE | wxDD_DIR_MUST_EXIST);
        if ( wxID_OK == picker.ShowModal() )
        {
            g_LibPath = picker.GetPath();
            g_LibPath.Append( wxT("\\") );
        }
        else
        {
            wxExit();
...
Application::OnInit()
{
    ...
    {
        printf(
            "Usage: FragmentShader [options] [filename]\n"
            "\n"
            "Options:\n"
            "  -h or --help\n"
            "    Show this help.\n"
